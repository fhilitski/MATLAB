      ...
      if numel(p)>1
        if i == numel(p) || numel(p)==2
          plot( [0 l/2] * double( cos(p(i).o+pi) ) + double( p(i).x(1) ), ...
                [0 l/2] * double( sin(p(i).o+pi) ) + double( p(i).x(2) ), [ c{1} '-' ],'LineWidth',4 );
        elseif i == 1
          plot( [0 l/2] * double( cos(p(i).o) ) + double( p(i).x(1) ), ...
                [0 l/2] * double( sin(p(i).o) ) + double( p(i).x(2) ), [ c{1} '-' ],'LineWidth',4 );  
        else
          plot( [-l/2 +l/2] * double( cos(p(i).o+pi) ) + double( p(i).x(1) ), ...
                [-l/2 +l/2] * double( sin(p(i).o+pi) ) + double( p(i).x(2) ), [ c{1} '-' ],'LineWidth',4 );
        end
        plot( double( p(i).x(1) ), double( p(i).x(2) ), [ c{2} 'x' ], 'MarkerSize', 30, 'LineWidth',4 );
      else
        plot( double( p(i).x(1) ), double( p(i).x(2) ), [ c{2} '+' ], 'MarkerSize', 30, 'LineWidth',4 );
      end
    end
    hold off
  end
