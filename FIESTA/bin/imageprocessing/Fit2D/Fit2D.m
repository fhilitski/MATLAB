...
%   region      the region of the image that has been used for fitting
  ...
  for i = 1:num_points
    switch modelstr(i)
      ...
      case 'b'
        fit_model{i} = Model2DGaussStretched( guess(i) );
      case 'r'
  ...
  global fit_pic xg yg; % use global variables to speed up calculation
  fit_pic = imcrop( pic, data.rect ); %<< create cropped image

  % calculate meshgrid once and store it in global variables
  [ xg, yg ] = meshgrid( 1:size( fit_pic, 2 ), 1:size( fit_pic, 1 ) );
  ...
  if bw_id > 0
    fit_bw = imcrop( bw, data.rect ); % create cropped image
    fit_bw = double( fit_bw == 0 | fit_bw == bw_id );
    fit_pic = fit_pic .* fit_bw + data.background * ( 1 - fit_bw );
  end
  ...
  if  minimum < Inf % at least one fit went well
    ...
    if all( x >= lb ) && all( x <= ub ) % parameters are not allowed to be at their bounds or beyond
      ...
      CoD = 1 - sum( residual(:).^2 ) / sum( ( fit_pic(:) - mean( fit_pic(:) ) ).^2 );

      % determine reduced chi squared
      reduced_chi = resnorm(idx) / ( numel(xg) - numel(x0) );
      ...
      error_events.fit_hit_bounds = error_events.fit_hit_bounds + 1;
      CoD = -10;
    end 
    ...
    switch exitflag
      case -2
        error_events.fit_hit_bounds = error_events.fit_hit_bounds + 1;
      case -4
        error_events.fit_impossible = error_events.fit_impossible + 1;
    end
...
%    surf( reshape( double(fit_pic), data.rect([4 3])+1 ) );
...
%  end
  ...
  % results: this function just passes the results of 'lsqnonlin'
    ...
    if supportsDerivative % models may calculate derivatives
      ...
      try
        ...
        rdev = Inf;
        rexitflag = -1;
        rresidual = Inf( size( fit_pic ) );
        routput = [];
        rlambda = [];
        rjacobian = zeros( numel( fit_pic ), numel( x0 ) );
        % check error
  ...
  %FITERROR calculates the difference between the data in the image 'fit_pic' and
  ...
  %   f   the difference of each point of the picture to the model
    ...
    for i_m = 1:numel(fit_model)
      f = f - evaluate( fit_model{i_m}, x(ids(i_m):ids(i_m+1)-1) );
    end
  ...
  %'fit_pic' and the theoretical model given in 'fit_model' using the 
  ...
  %   xb  the jacobian of 'f' with respect to 'x'
    if numel(fit_model)>1
        w = zeros([numel(fit_pic) numel(fit_model)]);
        for i_m = 1:numel(fit_model)
    ...
    if nargout == 1 % only function value requested
      ...
      xb = zeros( numel(xg), numel(x) ); % allocate memory for jacobian
      f = fit_pic - x(1); % first subtract background
      ...
      for i_m = 1:numel(fit_model)
        try
            [ df, xb( :, ids(i_m):ids(i_m+1)-1 ) ] = ...
              evaluate( fit_model{i_m}, x(ids(i_m):ids(i_m+1)-1) );
        catch
            df = evaluate( fit_model{i_m}, x(ids(i_m):ids(i_m+1)-1) );
        end
        f = f - w(:,i_m).*df;
      end
    end
