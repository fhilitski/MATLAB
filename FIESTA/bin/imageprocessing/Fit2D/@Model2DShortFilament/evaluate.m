function [ f, xb ] = evaluate( model, x )
  ...
  if nargout == 1 % calculate value of function
    ...
    w1 = HalfPlane( [ x(1:2)  a + pi/2 ] );
    w2 = HalfPlane( [ x(3:4)  a - pi/2 ] );
    g = GaussianStrip( [ x(1:2)  a  x(5) ] );
    f = x(6) .* ( ...
      w1 .* exp( -( (xg-x(1)).^2 + (yg-x(2)).^2 ) * x(5) ) + ...
      w2 .* exp( -( (xg-x(3)).^2 + (yg-x(4)).^2 ) * x(5) ) + ...
      ( 1 - w1 ) .* ( 1 - w2 ) .* g );
    ...
    % call subfunctions
    [ w1, wb1 ] = HalfPlane( [ x(1:2)  a + pi/2 ] );
    [ w2, wb2 ] = HalfPlane( [ x(3:4)  a - pi/2 ] );
    [ g, gb ] = GaussianStrip( [ x(1:2)  a  x(5) ] );
    
    % forward calculation
    temp = ( xg-x(3) ).^2 + ( yg-x(4) ).^2 ;
    temp3 = exp( -x(5) * temp );
    temp0 = ( xg-x(1) ).^2 + ( yg-x(2) ).^2 ;
    temp1 = exp( -x(5) * temp0 );
