...
if FileName~=0
    ...
    if FilterIndex==1
        ...
        fprintf(f,['Velocity(mean): ' num2str(mean(cell2mat(Data.vel)),Units.fmt) ' ' char(177) ' ' num2str(std(cell2mat(Data.vel)),Units.fmt) ' ' Units.str '/s (SD)\n']);
        if isfield(Data,'fit')
            params = coeffvalues(Data.fit);
            for n = 1:round(length(params)/3);
                fprintf(f,['Velocity(peak' num2str(n) '): ' num2str(params(3*n-1),Units.fmt) ' ' char(177) ' ' num2str(params(n*3)/sqrt(2),Units.fmt) ' ' Units.str '/s (SD)\n']); 
            end
        end
        fprintf(f,'\n');
...
if FileName~=0
    ...
    if isfield(Data,'fit')
        ...
        for m = 1:length(params)/3
            text(params(m*3-1),f(params(m*3-1))+max(Data.hist(:,2))*0.05,['Velocity: ' num2str(params(m*3-1),Units.fmt) ' ' char(177) ' ' num2str(params(m*3)/sqrt(2),Units.fmt) ' nm/s (SD)'],'VerticalAlignment','Bottom','HorizontalAlignment','center','Tag','fit','FontSize',12,'FontWeight','normal','BackgroundColor','w');   
        end
...
switch get(hVelocityStatsGui.mMethod,'Value')
            ...
            if size(Objects(n).Results,1)>1
                if get(hVelocityStatsGui.mData,'Value')==1
                    ...
                    if ~isempty(Objects(n).PathData)
                        if get(hVelocityStatsGui.mSmooth,'Value')==1 || isempty(hVelocityStatsGui.SmoothBox)
                            vel{n} = PointPointVelocity1D(Objects(n).Results(:,2),real(Objects(n).PathData(:,3)));
                        else
                            vel{n} = PointPointVelocity1D(Objects(n).Results(:,2),smooth(real(Objects(n).PathData(:,3)),hVelocityStatsGui.SmoothBox));
                        end
                    end
                end
                time{n} = Objects(n).Results(:,2)';
        ...
        for n = 1:length(Objects) 
            ...
            if size(Objects(n).Results,1)>1
                if get(hVelocityStatsGui.mData,'Value')==1
                    ...
                    if ~isempty(Objects(n).PathData)
                        if get(hVelocityStatsGui.mSmooth,'Value')==1 || isempty(hVelocityStatsGui.SmoothBox)
                            f = fit(double(Objects(n).Results(:,2)),double(real(Objects(n).PathData(:,3))),'poly1');
                        else
                            f = fit(double(Objects(n).Results(:,2)),smooth(double(real(Objects(n).PathData(:,3))),hVelocityStatsGui.SmoothBox),'poly1');
                        end
                    end
                end
            end
    ...
    case 4
        progressdlg('String','Calculating velocities','Min',0,'Max',length(Objects),'Parent',hVelocityStatsGui.fig);
        for n = 1:length(Objects)  
            vel{n}=[];
            time{n}=[];
            if size(Objects(n).Results,1)>1 
                if get(hVelocityStatsGui.mData,'Value')==1
                    ...
                    if ~isempty(Objects(n).PathData)
                        if get(hVelocityStatsGui.mSmooth,'Value')==1 || isempty(hVelocityStatsGui.SmoothBox)
                            vel{n} = Fit1D(Objects(n).Results(:,2),real(Objects(n).PathData(:,3)),hVelocityStatsGui.FitInterval);
                        else
                            vel{n} = Fit1D(Objects(n).Results(:,2),smooth(real(Objects(n).PathData(:,3)),hVelocityStatsGui.SmoothBox),hVelocityStatsGui.FitInterval);
                        end
                    end
                end
                time{n} = Objects(n).Results(:,2)';
    ...
    switch get(hVelocityStatsGui.mMethod,'Value')
        ...
        case 3
            if isempty(Data.vel{nObject})
                ...
                if get(hVelocityStatsGui.mData,'Value')==1
                    if get(hVelocityStatsGui.mSmooth,'Value')==1 || isempty(hVelocityStatsGui.SmoothBox)
                        plot(hVelocityStatsGui.aPlot,Objects(nObject).Results(:,2),Objects(nObject).Results(:,5)/Units.val,'b-');
                    else
                        plot(hVelocityStatsGui.aPlot,Objects(nObject).Results(:,2),smooth(Objects(nObject).Results(:,5),hVelocityStatsGui.SmoothBox)/Units.val,'b-');
                    end
                else
                    if get(hVelocityStatsGui.mSmooth,'Value')==1 || isempty(hVelocityStatsGui.SmoothBox)
                        plot(hVelocityStatsGui.aPlot,Objects(nObject).Results(:,2),real(Objects(nObject).PathData(:,3))/Units.val,'b-');
                    else
                        plot(hVelocityStatsGui.aPlot,Objects(nObject).Results(:,2),smooth(real(Objects(nObject).PathData(:,3)),hVelocityStatsGui.SmoothBox)/Units.val,'b-');
                    end
                end
                hold(hVelocityStatsGui.aPlot,'on');
                plot(hVelocityStatsGui.aPlot,Objects(nObject).Results(:,2),Data.vel{nObject}*Objects(nObject).Results(:,2)+Data.time{nObject}/Units.val,'r-');
...
if p>1
    ...
    set(h,'Tag','fit');
    for m = 1:length(x)
        text(params(m*3-1),f(params(m*3-1))+max(n)*0.05,['Velocity: ' num2str(params(m*3-1),Units.fmt) ' ' char(177) ' ' num2str(params(m*3)/sqrt(2),Units.fmt) ' nm/s (SD)'],'VerticalAlignment','Bottom','HorizontalAlignment','center','Tag','fit','FontSize',12,'FontWeight','bold','BackgroundColor','w');   
    end
    set(hVelocityStatsGui.aResults,{'xlim','ylim'},Data.xy);
