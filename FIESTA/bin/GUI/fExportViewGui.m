...
if strcmp(get(hMainGui.ToolBar.ToolKymoGraph,'State'),'on')
    ...
    if mean(T(2:end)-T(1:end-1))>10
        if T(end)-T(1)>=1000000
            T=T/60;
    ...
    T = T/1000;
    hExportViewGui.PixTime = mean(T(2:end)-T(1:end-1));
else
...
if get(hExportViewGui.cScale,'Value')
    ...
    hExportViewGui.BarLabel=text('Parent',hExportViewGui.PlotAxis,'Position',abs(pos(1:2)),'HorizontalAlignment','center','VerticalAlignment',align,'Color','white',...
                                 'String',[num2str(BarSize)  char(181) 'm'],'Visible','on','FontWeight','bold','FontUnits','points','FontSize',pos(3)*50,'FontName','Arial');                  
    if ~isempty(hExportViewGui.PixTime)
        ...
        hExportViewGui.BarLabel(2)=text('Parent',hExportViewGui.PlotAxis,'Position',abs(pos),'HorizontalAlignment','center','VerticalAlignment',align,'Color','white','Rotation',90,...
                                        'String',sprintf(hExportViewGui.time_format,TimeBar),'Visible','on','FontWeight','bold','FontUnits','points','FontSize',50,'FontName','Arial');                 
    end
...
if get(hExportViewGui.cTime,'Value')&&strcmp(get(hExportViewGui.cTime,'Enable'),'on')&&~get(hExportViewGui.rCurrentView,'Value')
    ...
    hExportViewGui.TimeStamp=text('Parent',hMainGui.MidPanel.aView,'Position',CalcTimeStamp(hExportViewGui),'HorizontalAlignment','center','Color','white',...
                            'String',sprintf(time_format,time),'Visible','on','FontWeight','bold','FontUnits','points','FontSize',50,'FontName','Arial');
...
if ~isempty(hExportViewGui.Region)&&nargin==1
    xy{1} = [min(hExportViewGui.Region.X) max(hExportViewGui.Region.X)];
    xy{2} = [min(hExportViewGui.Region.Y) max(hExportViewGui.Region.Y)];
end
x_total=xy{1}(2)-xy{1}(1);
y_total=xy{2}(2)-xy{2}(1); 
...
if nargin==1
    ...
    if get(hExportViewGui.mPosBar,'Value')==1||get(hExportViewGui.mPosBar,'Value')==3
        x=xy{1}(1)+0.05*s+width/2;
    else
        x=xy{1}(2)-0.05*s-width/2;
    end
    if get(hExportViewGui.mPosBar,'Value')==1||get(hExportViewGui.mPosBar,'Value')==2
        y=xy{2}(1)+0.05*s+height;
    else
        y=-(xy{2}(2)-0.05*s-height);
    end          
    pos=[x y s/st];
    ...
    height=varargin{2}/hExportViewGui.PixTime;
    if get(hExportViewGui.mPosBar,'Value')==1||get(hExportViewGui.mPosBar,'Value')==3
        x = xy{1}(1)+0.05*s+width;
    else
        x = -(xy{1}(2)-0.05*s-width);
    end
    if get(hExportViewGui.mPosBar,'Value')==1||get(hExportViewGui.mPosBar,'Value')==2
        y=xy{2}(1)+0.05*s+height/2;
    else
        y=xy{2}(2)-0.05*s-height/2;
    end          
    pos=[x y];
end
...
%I = print(hMainGui.fig, '-RGBImage');
Frame = getframe(hMainGui.fig);
I = Frame.cdata;
y = size(I,1);
x = size(I,2);
...
if isempty(hExportViewGui.Region)
    if xy{1}(1)<0
        I = I(:,ceil(abs(xy{1}(1)-0.5)/lx*x)+1:fix(x-abs(xy{1}(1)-0.5)/lx*x)-3,:);
    end
    if xy{2}(1)<0
        I = I(ceil(abs(xy{2}(1)-0.5)/ly*y)+4:fix(y-ceil(abs(xy{2}(1)-0.5)/ly*y)),:,:);
    end
else
    xy2{1} = [min(hExportViewGui.Region.X) max(hExportViewGui.Region.X)];
    xy2{2} = [min(hExportViewGui.Region.Y) max(hExportViewGui.Region.Y)];
    I = I(:,ceil(abs(xy{1}(1)-xy2{1}(1))/lx*x)+1:fix(x-abs(xy{1}(2)-xy2{1}(2))/lx*x)-1,:);
    I = I(ceil(abs(xy{2}(1)-xy2{2}(1))/lx*x)+1:fix(y-abs(xy{2}(2)-xy2{2}(2))/ly*y)-1,:,:);
end
