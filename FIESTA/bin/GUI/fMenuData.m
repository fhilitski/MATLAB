...
if PathName~=0
    ...
    f=[PathName FileName];
    try
        if strcmp(filetype,'ND2')
            [Stack,TimeInfo]=fReadND2(f); 
        else
            [Stack,TimeInfo]=fStackRead(f);
        end
    catch ME   
        fMsgDlg(ME.message,'error');
        failed=1;
    end
    if isnan(Config.Time) && (isempty(TimeInfo{1}) || length(unique(TimeInfo{1}))<length(TimeInfo{1}))   
    ...
    if strcmp(fOpenStruct.Mode,'rSeparateFiles')
        ...
        for n = 1:nChannels
            ...
            elseif strcmpi(FileName(end-3:end),'.nd2')
                ...
                if nFrames(n)>1
                    Config.Time(n) = str2double(fInputDlg('Enter plane time difference in ms:','100'));
                else
                    Config.Time(n) = 0;
                end
                filetype{n} = 'TIFF'; 
