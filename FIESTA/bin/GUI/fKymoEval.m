...
if ~isempty(Stack)
    ...
    if ~isempty(hKymoEval.ref) && ~isempty(mode)
        ...
        Image = imadjust(uint16(hKymoEval.ref),[double(min(min(hKymoEval.ref)))/2^16 double(mContrast)/2^16],[]);
...
if strcmp(mode,'Create')
    Image = imadjust(hKymoEval.ref,[double(min(min(hKymoEval.ref)))/2^16 double(mContrast)/2^16],[]);
else
    ...
    if  all(cp>=[xy{1}(1) xy{2}(1)]) && all(cp<=[xy{1}(2) xy{2}(2)])
        if isempty(hKymoEval.cp)   
            ...
            elseif measure == hKymoEval.rAutomatic
                ...
                fMsgDlg('Choose either manuell or automatic measurements','warn');
            end
        else
...
line([0.5 total_x+0.5 total_x+0.5 0.5 0.5],[0.5 0.5 total_y+0.5 total_y+0.5 0.5],'Color','red','LineWidth',2,'Tag','border','Parent',hKymoEval.aImage);
hLine = findobj('Tag','line');
