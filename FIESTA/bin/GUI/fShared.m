    ...
    for m = stidx
        ...
        if ~isempty(k)
            ...
            for i=k
                ...
                if isfield(Molecule,'Data')&&~isempty(Molecule(i).Data)
                    R_Z = Molecule(i).Data;
                    DisZ=zeros(n,1);
                end
                DisX=zeros(n,1);
            ...
            if isfield(Molecule,'Data')&&~isempty(Molecule(i).Data)
                ...
                Drift{m}=[F' drift_x drift_y drift_dx drift_dy];   
            end
        end
    end
...
if isfield(Config,'DynamicFil')
    if Config.DynamicFil
        ...
        params = struct('threshold',hMainGui.Values.Thresh(1),'scale',Config.PixSize,'fwhm_estimate',Config.Threshold.FWHM/Config.PixSize,'binary_image_processing',filter,'background_filter',background);
    ...
    for n = batch
        ...
        if (~isempty(Stack)||strcmp(Mode,'Reconnect'))   
            ...
            if strcmp(addConfig.Threshold.Mode,'relative')==1
                addConfig.Threshold.Value = hMainGui.Values.RelThresh(k)*1i;
