...
if ~isempty(PathStats)
    if strcmp(button,'Filament')
        [XPosFil,YPosFil,FrameFil,LengthFil]=InterpolFil(Filament);
    else
        XPosFil = [];
        YPosFil = [];
        FrameFil = [];
    ...
    setappdata(hPathsStatsGui.fig,'LengthFil',LengthFil);  
        ...
        if isempty(PathStats(n).PathData)
            if size(PathStats(n).Results,1)>9&&~strcmp(button,'Filament')
                ...
                elseif strcmp(button,'Average')
                    if isfield(PathStats,'Data')
                        Data = PathStats(n).Data;
                    else
                        Data =[];
                    end
                    [XYZ,Dis,Side] = AveragePath(PathStats(n).Results(:,1:4),AverageDis,Data);
                    PathX = XYZ(:,1);
                    PathY = XYZ(:,2);
                    if size(XYZ,2)>2
                        PathZ = XYZ(:,3);
            ...
            elseif strcmp(button,'Filament')
                ...
                [PathX,PathY,Dis,Side]=FilamentPath(PathStats(n).Results,XPosFil(k,:),YPosFil(k,:),FrameFil{k},1,LengthFil{k});
...
for n = 1:nFil
    for m = 1:length(Filament(n).Data)
        ...
        P = 1:length(X);
        pi = -5:0.01:length(X)+5;
        xi{n,m} = interp1(P,X,pi,'linear','extrap'); %#ok<AGROW>
        yi{n,m} = interp1(P,Y,pi,'linear','extrap'); %#ok<AGROW>
    end
    fi{n} = Filament(n).Results(:,1);
    fL{n} = Filament(n).Results(:,6);
    h=progressdlg(n);
    if isempty(h)
        return
    end 
        ...
        if gcbo==hPathsStatsGui.bAuto
            ...
            elseif get(hPathsStatsGui.rPoly3,'Value')==1
             ...
             elseif get(hPathsStatsGui.rFilament,'Value')==1
                ...
                LengthFil = getappdata(hPathsStatsGui.fig,'LengthFil');
                if isempty(XPosFil)
                    [XPosFil,YPosFil,FrameFil,LengthFil]=InterpolFil(Filament);
                end
                nFil = size(XPosFil,1);
                ...
                if ~isempty(get(hPathsStatsGui.eAverage,'String'))
                    ...
                    [XYZ,Dis,Side] = AveragePath(PathStats(i).Results(:,1:4),round(str2double(get(hPathsStatsGui.eAverage,'String'))),Data);
                    PathX = XYZ(:,1);
                    PathY = XYZ(:,2);
                    if size(XYZ,2)>2
                        PathZ = XYZ(:,3);
...
Dis = real(PathStats(idx).PathData(:,3));
...
function [XYZ,Dis,Side]=AveragePath(Results,DisRegion,PosZ)
nData=size(Results,1);
...
[XYZ,~,Dis] = distance2curve(NRes(:,2:4),[PosX PosY PosZ],'spline');
