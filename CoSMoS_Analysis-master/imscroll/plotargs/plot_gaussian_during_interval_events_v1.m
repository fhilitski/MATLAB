function pc=plot_gaussian_during_interval_events_v1(Intervals, aoifits, min_frmlength, max_frmlength, increase_start, decrease_end, fignum)
%
% function plot_gaussian_during_interval_events_v1(Intervals, aoifits, minfrmlength, max_frmlength, increase_start, decrease_end, fignum)
%
...
figure(fignum);
%dat=draw_aoifits_aois_v1(aoifits,'y');       
...
for aoiindx=1:aoinum
        ...
        % Cycle through the aoicells
    [highrose highcol]=size(aoicells{aoiindx,2});    % highrose = # of high events for aoi#=aoiindx 
    logik=aoifits.data(:,1)==aoicells{aoiindx,3};        % Pick out lines matching the aoi number
                                                        % for this aoicells
    dat=aoifits.data(logik,:);
    for eventindx=1:highrose
        ...
        logik=(dat(:,2)>=frmstart+increase_start)&(dat(:,2)<=frmend-decrease_end);
            ...
            if ( (sum(logik)>=min_frmlength)& (sum(logik)<=max_frmlength) )
               ...
               %pccount, frmend, frmstart, increase_start, decrease_end, size(pc(pccount:(pccount+(frmend-frmstart-increase_start-decrease_end)),:)),size(dat(logik,:))
                pc(pccount:(pccount+(frmend-frmstart-increase_start-decrease_end)),:)=dat(logik,:);
