function pc=gauss2d_mapstruc(dum,images,folder,mapstruc,parenthandles)
%
% function gauss2d_mapstruc(images,mapstruc,parenthandles,handles)
%
...
firstaoi=firstfrm(ylow:yhi,xlow:xhi);
 ...
 inputarg0=[mx-mn pixnum/2 pixnum/2 pixnum/4 mn]; 
  switch (get(parenthandles.FitChoice,'Value'))
      ...
      pc=[mapstruc(1).aoiinf(1) outarg(1) outarg(2)+xlow-1 outarg(3)+ylow-1 outarg(4) outarg(5) sum(sum(firstaoi))];
      case 2
     % Here if we only integrate the aoi, not fitting
      ...
      pc=[pc;mapstruc(1).aoiinf(1:5) 0 sum(sum(firstaoi))];
      ...
      pc=[pc;mapstruc(1).aoiinf(1) 0 maoix maoiy 0 0 sum(sum(mfirstaoi))];
      ...
      pc=[mapstruc(1).aoiinf(1) outarg(1) outarg(2)+xlow-1 outarg(3) outarg(4)+ylow-1 outarg(5) outarg(6) sum(sum(firstaoi))];
  ...
  for framemapindx=2:ncol
      ...
      if framemapindx/20==round(framemapindx/20)
     ...
     [mpc npc]=size(pc);                  % Get the last outputs
     lastoutput=pc(mpc,:);
     ...
     switch (get(parenthandles.FitChoice,'Value'))
         ...
         pc=[pc;mapstruc(framemapindx).aoiinf(1) outarg(1) outarg(2)+xlow-1 outarg(3)+ylow-1 outarg(4) outarg(5) sum(sum(currentaoi))];
         case 2
         % Here if we only integrate the aoi, not fitting
                               % the spot to a gaussian.  Note that we
                               % retain the original aoi coordinates, but
                               % have a zero offset in our output matrix
         pc=[pc;mapstruc(framemapindx).aoiinf(1:5) 0 sum(sum(currentaoi))];
         ...
         mcurrentaoi=currentfrm(mylow:myhi,mxlow:mxhi);
                % [frame# amp xcenter ycenter sigma offset (int aoi)]
                % (aoi# added later as first element in each row)
         pc=[pc;mapstruc(framemapindx).aoiinf(1) 0 maoix maoiy 0 0 sum(sum(mcurrentaoi))];
        ...
        pc=[pc;mapstruc(framemapindx).aoiinf(1) outarg(1) outarg(2)+xlow-1 outarg(3) outarg(4)+ylow-1 outarg(5) outarg(6) sum(sum(currentaoi))];
